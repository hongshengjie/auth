...
func NewAuditLogEntry(r *http.Request, tx *surrealdb.DB, actor *User, action AuditAction, ipAddress string, traits map[string]interface{}) error {
	...
	if _, err := surrealdb.Insert[AuditLogEntry](tx, models.Table("audit_log_entries"), l); err != nil {
...
func FindAuditLogEntries(tx *surrealdb.DB, filterColumns []string, filterValue string, pageParams *Pagination) ([]*AuditLogEntry, error) {
	rets, err := surrealdb.Query[AuditLogEntry](tx, "select * from ddd where", nil)
	if err != nil {
		return nil, err
	}
	var retss []*AuditLogEntry
	for _, v := range *rets {
		retss = append(retss, &v.Result)
	}
	return retss, nil
